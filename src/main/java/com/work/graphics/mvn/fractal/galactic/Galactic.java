 ...
 * @author linux
 */
public class Galactic extends JPanel {

    private static final double REALMIN = -0.25;
    private static final double IMAGMIN = -0.25;
    private static final double REALMAX = 0.25;
    private static final double IMAGMAX = 0.25;
    ...
    private Palette[] load(String fileName) {
        ...
        try {
            ...
            Logger.getLogger(Galactic.class.getName()).log(Level.SEVERE, null, ex);
        }
        return color;
    }
    ...
    private void draw(Graphics2D g2d) {
        ...
        for (int i = 0; i < maxx; ++i) {
            for (int j = 0; j < maxy; ++j) {
                ...
                z = Complex.ofCartesian(((REALMAX - REALMIN) / (maxx)) * i + REALMIN, 
                        ((IMAGMAX - IMAGMIN) / (maxy)) * j + IMAGMIN);
                //iniciamos Z=-0.7+0.775i
                c = Complex.ofCartesian(-0.7, 0.775);
                //inicializamos el contador
                cont = 0;

                do {
                    //Z=C*Z - C*Z*Z; --> ecuacion Z=C*Z(1-Z)
                    Complex num1 = c.multiply(z);
                    Complex num2 = c.multiply(z).multiply(z);
                    z = num1.subtract(num2);
                    cont++;
